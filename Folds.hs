...
instance Show TransformationAST where
    show (T transformation) = case (T transformation) of
      (T (Translation dx dy)) -> "+(" ++ show dx ++ "," ++ show dy ++ ")"
      (T (Scaling factor )) ->"*<" ++ show factor ++ ">"
      -- aplic map show pentru ca avem mai multe transformari
      (T (Combine transformations)) -> "["++ intercalate ","(map show transformations) ++"]"
    ...
    show = showRegion 0
      where
        ...
        showRegion level (R shape) = case shape of
            ...
            Complement reg -> indent level ++ "~\n" ++ showRegion (level + 2) reg
            Union reg1 reg2 -> indent level ++ "+\n" ++ showRegion (level + 2) reg1 ++ "\n" ++ showRegion (level + 2) reg2
            Intersection reg1 reg2 -> indent level ++ "*\n" ++ showRegion (level + 2) reg1 ++ "\n" ++ showRegion (level + 2) reg2
            Transform trans reg -> indent level ++ show trans ++ "\n" ++ showRegion (level + 2) reg
        
        -- functie ajutatoare care calculeaza indentarea
        indent :: Int -> String
...
instance Functor TransformationShape where
    ...
    fmap f transformation = case transformation of
        ...
        -- aplicam functia f pe xs
        (Combine xs) -> Combine (map f xs)
...
instance Functor RegionShape where
    ...
    fmap f region = case region of
        ...
        (Transform trans reg) -> Transform trans (f reg)
...
-- aplic functia fold... cu fmap pentru toate transformarile care pot fi liste
-- aplic combinerul f pe rezultatul lui fmamp pentru a reduce la a
...
-- aplic functia fold... recursiv cu fmap pentru toate regiunile
...
basicTransformationCount = foldTransformationAST combiner
  where
    ...
    combiner transformation = case transformation of
        (Translation dx dy) -> 1
        (Scaling factor) -> 1
        -- suma transformarilor
        (Combine transformation) -> sum transformation
...
basicEntityCount = foldRegionAST combiner
  where
    ...
    combiner region = case region of
        (FromPoints points) -> (1, 0)
        (Rectangle w h) -> (1, 0)
        (Circle r) -> (1, 0)
        (Complement reg) -> reg
        (Union reg1 reg2) -> (fst reg1 + fst reg2, snd reg1 + snd reg2)
        (Intersection reg1 reg2) -> (fst reg1 + fst reg2, snd reg1 + snd reg2)
        (Transform trans reg) -> (fst reg, snd reg + basicTransformationCount trans)
