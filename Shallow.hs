...
rectangle width height (x, y)
    | x >= -width / 2 && x <= width / 2 && y <= height / 2 && y >= -height/2 = True
    | otherwise = False
...
bfs start expand = bfsHelp S.empty [(start, 0)]
  where
    -- bfsHelp este o functie auxiliara unde implementez logica bfs
    -- in visited tin nodurile vizitate
    -- daca coada este goala returnam o lista vida
    bfsHelp visited [] = []
    bfsHelp visited ((curr, dist) : xs)
        -- verific daca fiecare nod din coada a fost vizitat
        -- daca a fost vizitat trec peste ek
        | curr `S.member` visited = bfsHelp visited xs
        -- altfel inseram nodul curent la multimea de noduri vizitate
        -- folosesc expand pentru gasirea tuturor vecinilor din nodul curent
        -- adaugam vecinii la coada de noduri de vizitat si distantele de la sursa la dest
        | otherwise = (curr, dist) : bfsHelp nodeVisited newQueue 
       where 
         nodeVisited = S.insert curr visited
         newQueue = xs ++ map (\s -> (s, dist + 1)) (expand curr)
...
regionAvoidingBfs start region = bfs start (expandRegion region)
    where
      -- generez toti vecinii unui punct si elimin punctele care 
      -- se afla in regiunea primita ca parametru filtrandu le pe cele care nu se afla in regiune
      -- adica cele de care avem nevoie pe parcurs deplansand punctul la sud, est, nord, vest
      expandRegion :: Region -> Point -> [Point]
      expandRegion region (x, y)  = filter (not . region) [(x, y-1), (x+1, y), (x, y+1), (x-1, y)]
