...
toTransformation transformation = case transformation of
    -- daca transformarea e translatie folosesc functia din Shallow
    (Translation dx dy) -> S.translation dx dy
    (Scaling factor) -> S.scaling factor
    -- dacă este o combinație de mai multe transformări
    -- recursiv aplic toTransformation fiecărei transformări
    -- combin rezultatele folosind combineTransformations din modulul Shallow
    (Combine transformations) ->
        S.combineTransformations $ map toTransformation transformations
{-
    *** TODO ***
...
toRegion regionAst = case regionAst of
    -- transformă o listă de puncte într-o regiune
    (FromPoints points) -> S.fromPoints points
    (Rectangle w h) -> S.rectangle w h
    (Circle r) -> S.circle r
    -- aplic recursiv toRegion pentru a obține
    -- funcția caracteristică a regiunii originale și aplic functia din Shallow
    (Complement region) -> S.complement(toRegion region)
    (Union reg1 reg2) -> S.union (toRegion reg1) (toRegion reg2)
    (Intersection reg1 reg2) -> S.intersection (toRegion reg1) (toRegion reg2)
    (Transform trans reg) -> S.applyTransformation (toTransformation trans) (toRegion reg)
...
-- Decompose recursiv fiecare transformare din listă
-- concatMap este folosită pentru a aplica decomposeTransformation fiecărui element
-- din lista de transformări și apoi pune tot intr o lista
...
fuseTransformations = reverse . foldl fuse []
  where
    fuse acc (Translation dx1 dy1)
    -- când ultimul element din acumulator este tot o translație,
    -- adun deplasările pe x și y
    -- dacă nu există , adaug transformarea  la acumulator
      | (Translation dx2 dy2 : rest) <- acc = Translation (dx1 + dx2) (dy1 + dy2) : rest
      | otherwise = Translation dx1 dy1 : acc

    fuse acc (Scaling s1)
      | (Scaling s2 : rest) <- acc = Scaling (s1 * s2) : rest
      | otherwise = Scaling s1 : acc
    -- pentru alt tip de transformare care nu se potrivește
    -- adaug transformarea la acumulator
    fuse acc trans = trans : acc
{-
    *** TODO ***
